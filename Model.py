...
df.dropna(subset=[SNR_COLUMN], inplace=True) # Handle potential non-numeric SNR values
...
fit_final_params = {
    ...
    'verbose': False # Suppress XGBoost training output for early stopping
}
...
plt.plot(lims, lims, 'r--', alpha=0.75, zorder=0, label='Perfect Prediction Line (y=x)')
plt.xlabel("Actual EW_r")
plt.ylabel("Predicted EW_r")
plt.title("Actual vs. Predicted EW_r (Test Set)")
plt.legend()
plt.grid(True)
plt.tight_layout()
...
plt.close()
print("SHAP bar plot saved to shap_feature_importance_bar.png")

# SHAP Dot Plot
plt.figure()
shap.summary_plot(shap_values, X_test_scaled_df, show=False)
plt.title("SHAP Summary Plot (Impact on EW_r Prediction)")
plt.tight_layout()
plt.savefig("shap_summary_dot_plot.png")
plt.close()
print("SHAP summary plot saved to shap_summary_dot_plot.png")
